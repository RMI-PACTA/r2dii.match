...
`r2dii.match` allows you to match loans from your loanbook to the companies in an asset-level dataset, but it is unlikely that you will be able to match every loan. A company may not be active in the sectors 2DII focuses on: `r sector_in_scope`; and even if the company does operate within 2DII's scope, the asset-level dataset may lack information about it. How can you measure how much of the loanbook matched some asset? This article shows two ways to calculate such matching coverage: (1) Calculate the portion of your loanbook covered, by dollar value (i.e. using one of the `loan_size_*` columns); and (2) Count the number of companies matched.
