...
`r2dii.match` allows you to match loans from your loanbook to the companies in an asset-level dataset, but it is unlikely that you will be able to match every loan. This article explores why, and shows how to calculate how much of the loanbook has been matched.
...
One reason why the matching might be partial is because some companies may not be active in the sectors 2DII focuses on: `r sector_in_scope`. 
...
We will use example datasets from `r2dii.data`. To demonstrate our point, we create a `loanbook` dataset with two mis-matching loans:
